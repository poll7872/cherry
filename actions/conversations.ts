...
import { Conversation, Message, ErrorResponseSchema } from "@/lib/schemas";
...
export async function createConversation(projectId: string, title: string): Promise<{ errors: string[]; data?: Conversation }> {
  const cookieStore = await cookies();
  const token = cookieStore.get(COOKIE_SESSION_NAME)?.value;
...
export async function sendMessage(conversationId: string, content: string): Promise<{ errors: string[]; message?: Message }> {
  const cookieStore = await cookies();
  const token = cookieStore.get(COOKIE_SESSION_NAME)?.value;
  ...
  try {
    ...
    const data = await response.json();
    // Revalidación opcional ya que el cliente actualizará el estado localmente
