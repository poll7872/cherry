...
export function DeleteConversationDialog({ 
  ...
  const handleDelete = () => {
    startTransition(async () => {
      const result = await deleteConversation(convId, projectId);
        ...
        <div className="p-10 pb-4 text-center space-y-8">
          ...
          <DialogHeader className="space-y-4">
            <DialogTitle className="text-3xl font-black text-white tracking-tight leading-tight italic">
              ¿Eliminar <span className="text-destructive not-italic">conversación?</span>
            </DialogTitle>
            <DialogDescription className="text-muted-foreground/60 font-sans leading-relaxed text-sm">
              Estás a punto de remover <span className="font-bold text-white/80">{convTitle}</span> de este workspace. Esta acción es irreversible y los datos se perderán permanentemente.
            </DialogDescription>
          </DialogHeader>
        </div>
